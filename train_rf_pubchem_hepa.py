...
def compute_fps(df, fingerprint="PubChem", path="descriptor_xml"):
    ...
    smiles_file = os.path.join(path, 'smiles_Hepa.smi')
    output_csv = os.path.join(path, f'{fingerprint}_Hepa.csv')
...
print("📥 Loading data...")
df = pd.read_csv("training_data/x_train_Hepa.csv")  # Must contain 'SMILES' and 'Label' columns
