...
def main():
    ...
    for x in ['AP2DC','AP2D','EState','CDKExt','CDK','CDKGraph','KRFPC','KRFP','MACCS','PubChem','SubFPC','SubFP', 'Desc']:
        print("#"*100) 
        x_train = pd.read_csv(os.path.join("Skin", "train", x+".csv"), index_col=0)
        y_train = pd.read_csv(os.path.join("Skin", "train", "y_train.csv"), index_col=0)
        x_test  = pd.read_csv(os.path.join("Skin", "test",  x+".csv"), index_col=0)
        y_test  = pd.read_csv(os.path.join("Skin", "test",  "y_test.csv"), index_col=0)
