...
def check_missing_smiles(df, smiles_col):
    ...
    df = df.drop(df[df['Molecule'].isna()].index)
...
def nanomolarconversion(df, ic50_col):
    ...
    return df 
def calculate_pic50(df, ic50_col, from_nM = True):
    #change nM to M and calculate log
    if from_nM == True:
        ic50_nM = df[ic50_col] * 1e-9
        df['pIC50'] = np.log10(ic50_nM) #take log of IC50
    else:
...
def lipinski_calculation_filter(df, canonical_smiles):
    #import RDKit
    from rdkit import Chem
    ...
    #check pass / fail drug likeness
    df['Pass'] = (df['MW'] <= 500) & (df['LogP'] <= 5) & (df['HBA'] <= 10) & (df['HBD'] <= 5)
    print("How many compound pass/fail", df['Pass'].value_counts())
...
def mean_sd_normality_shapiro(df, col):
    mean = statistics.mean( df[col])
    sd   = statistics.stdev(df[col])
    stat, p = shapiro(df[col])
    print(f'{col} mean: {round(mean,3)} +/- {round(sd,3)}, Statistics= {round(stat,3)}, p= {p}')
    return mean, sd, p